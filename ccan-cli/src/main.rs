...
fn run(args: Args) -> Result<()> {
    ...
    let cc_freqs_file = &create_path(&[
        output_dir.as_str(),
        format!("cc_freqs-a{a}-d{d}-c{c}-f{f}.csv").as_str(),
    ]);
    let cc_probs_file = &create_path(&[
        output_dir.as_str(),
        format!("cc_probs-a{a}-d{d}-c{c}-f{f}.csv").as_str(),
    ]);
    let cc_files_file = &create_path(&[
        output_dir.as_str(),
        format!("cc_files-a{a}-d{d}-c{c}-f{f}.csv").as_str(),
    ]);
    let c_data_file = &create_path(&[
        output_dir.as_str(),
    ...
    let predict_since =
        Utc::from_utc_datetime(&Utc, &args.predict_since.and_hms_opt(0, 0, 0).unwrap());
    let predict_until =
        Utc::from_utc_datetime(&Utc, &args.predict_until.and_hms_opt(23, 59, 59).unwrap());

    let file_filters = FileFilteringOpt::new(
        &[args.exclude_regex.as_str()],
        &[args.include_regex.as_str()],
    );
